...
// UUID - UniqueIDGeneration
import { v4 as uuidv4 } from 'uuid';

// DemoLabelData
const LABELS = [
  { id: 'l1', name: 'Design', color: '#a855f7' }, // Purple
  { id: 'l2', name: 'Dev', color: '#3b82f6' }, // Blue
  { id: 'l3', name: 'Urgent', color: '#ef4444' }, // Red
  { id: 'l4', name: 'Review', color: '#eab308' }, // Yellow
  { id: 'l5', name: 'Done', color: '#22c55e' }, // Green
  { id: 'l6', name: 'Research', color: '#14b8a6' }, // Teal
];
...
function boardReducer(state, action) {
  switch (action.type) {
    case ACTIONS.ADD_CARD: {
      const { listId, title } = action.payload; // ExtractFromPayload
      const newCard = {
        id: uuidv4(), // GenerateUniqueID - UUID
        title,
        description: '',
        labels: [],
        dueDate: null,
    ...
    case ACTIONS.UPDATE_CARD: {
      ...
      return {
        ...
        cards: {
          ...state.cards,
          [cardId]: {
            ...state.cards[cardId], // KeepOldCardData
            ...updates, // OverwriteNewData
          },
        },
      };
    }
    ...
    // MoveAListDragAndDrop
    case ACTIONS.MOVE_LIST: {
      const { sourceIndex, destIndex } = action.payload;
      const newOrder = [...state.listOrder]; // Copy
      const [moved] = newOrder.splice(sourceIndex, 1); // RemoveFromSource
      newOrder.splice(destIndex, 0, moved); // InsertAtDestination
    ...
    case ACTIONS.TOGGLE_CHECKLIST_ITEM: {
      ...
      return {
        ...
        cards: {
          ...
          [cardId]: {
            ...state.cards[cardId],
            checklist: state.cards[cardId].checklist.map(
              item =>
                item.id === itemId ? { ...item, done: !item.done } : item, // ToggleDone
            ),
          },
        },
      };
    ...
    case ACTIONS.OPEN_CARD_MODAL:
      return { ...state, openCardId: action.payload.cardId }; // OpenModal

    case ACTIONS.CLOSE_CARD_MODAL:
      return { ...state, openCardId: null }; // CloseModal

    default:
      return state; // ReturnUnchangedStateForUnknownAction
  }
}
...
export function BoardProvider({ children }) {
  ...
  const updateBoardTitle = useCallback(
    title => dispatch({ type: ACTIONS.UPDATE_BOARD_TITLE, payload: { title } }),
    [],
  );

  const openCardModal = useCallback(
    cardId => dispatch({ type: ACTIONS.OPEN_CARD_MODAL, payload: { cardId } }),
    [],
  );
  ...
  // ContextValue - WhatAllChildComponentsWillReceive
  const value = {
    // State
    ...state,
...
export function useBoard() {
  const context = useContext(BoardContext); // GetContextValue

  if (!context) {
    throw new Error('useBoard must be used within a BoardProvider');
  }

  return context; // ReturnContextValue
}
