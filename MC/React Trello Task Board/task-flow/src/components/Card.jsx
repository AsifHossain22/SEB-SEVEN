...
export default function Card({ card, listId }) {
  ...
  // HoverStateToShowActionButtons
  const [isHovered, setIsHovered] = useState(false);

  // SortableSetup - dnd-kit
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const totalChecks = card.checklist?.length ?? 0; // TotalItems
  const doneChecks = card.checklist?.filter(i => i.done).length ?? 0; // DoneItems
  const checkProgress =
    totalChecks > 0 ? Math.round((doneChecks / totalChecks) * 100) : 0; // Percentage
  const allDone = totalChecks > 0 && doneChecks === totalChecks; // AllDone?

  // HandleDueDateFormatAndStatus
  const getDueDateInfo = () => {
    if (!card.dueDate) return null; // NullIfNoDueDate

    const due = new Date(card.dueDate); // DueDateObject
    const now = new Date(); // CurrentTime
    const diff = due - now; // DifferenceInMilliSecond

    const isOverdue = diff < 0; // IsOverdue?
    const isDueSoon = diff > 0 && diff < 86400000; // Within 24 Hours?
  ...
  const dueDateInfo = getDueDateInfo(); // DueDateInfo

  // ExtractLabelObjectsForCard
  const cardLabels = (card.labels || [])
    .map(labelId => labels.find(l => l.id === labelId))
    .filter(Boolean); // FilterOutMissingLabels
  ...
  return (
    ...
    >
        ...
        {(card.description || dueDateInfo || totalChecks > 0) && (
          <div className="flex flex-wrap items-center gap-2 mt-2.5">
            ...
            {card.description && (
              ...
              >
                <AlignLeft size={11} /> {/* DescriptionIcon */}
              </div>
            )}
      ...
      <button
        ...
        onClick={() => openCardModal(card.id)} // মডাল খোলা | Open modal
