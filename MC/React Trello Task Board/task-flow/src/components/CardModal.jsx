...
export default function CardModal() {
  ...
  const saveTitle = () => {
    ...
    if (trimmed) {
      updateCard(card.id, { title: trimmed }); // UpdateCard
    } else {
      setEditTitle(card.title); // RestoreCardTitle
    }
  ...
  const handleAddCheckItem = () => {
    ...
    if (trimmed) {
      addChecklistItem(card.id, trimmed); // AddItem
    }
    setNewCheckItem(''); // ClearInput
  ...
  const toggleLabel = labelId => {
    ...
    const newLabels = currentLabels.includes(labelId)
      ? currentLabels.filter(id => id !== labelId) // RemoveIfExists
      : [...currentLabels, labelId]; // AddIfNoExists
    updateCard(card.id, { labels: newLabels }); // UpdateCard
  };

  // HandleSetCoverColor
  const setCoverColor = color => {
    updateCard(card.id, { coverColor: color }); // UpdateColor
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ModalHeader */}
        <div className="flex items-start gap-3 px-6 pt-5 pb-0 flex-shrink-0">
          {/* TitleIcon */}
          <Type size={18} className="text-slate-400 mt-1 flex-shrink-0" />
          ...
          <button
            ...
            className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700 transition-all flex-shrink-0"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
              ...
              {isEditingDesc ? (
                ...
                <div>
                  <textarea
                    ...
                    className="w-full bg-slate-800 text-sm text-slate-200 placeholder-slate-500 rounded-xl p-3 resize-none outline-none border border-brand-500 min-h-[100px]"
                    rows={4}
                    autoFocus
                  />
                ...
                <div
                  ...
                  className="min-h-[60px] bg-slate-800/50 hover:bg-slate-800 rounded-xl p-3 text-sm text-slate-400 cursor-text transition-colors"
                >
                  {card.description || (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all
                        ${
                          item.done
                            ? 'bg-brand-500 border-brand-500' // Checked
                            : 'border-slate-600 hover:border-brand-400' // Unchecked
                        }`}
                    >
                      {/* CheckMark */}
          ...
          {/* Sidebar */}
          <div
            className="w-44 flex-shrink-0 px-3 py-4 space-y-2 overflow-y-auto"
          ...
          >
            ...
            <div>
              <label className="flex items-center gap-2 w-full px-3 py-2 text-xs text-slate-300 bg-slate-800/70 hover:bg-slate-700 rounded-lg transition-colors cursor-pointer">
                ...
                <input
                  ...
                  className="sr-only" // VisuallyHiddenButFunctional
                />
              </label>
              {/* ShowCurrentDueDate */}
              {card.dueDate && (
            ...
            {/* DangerZoneDeleteCard */}
            <div className="pt-4 border-t border-slate-800 mt-4">
              <button
                onClick={() => {
                  deleteCard(card.id, listId); // DeleteCard
                  closeCardModal(); // CloseModal
                }}
